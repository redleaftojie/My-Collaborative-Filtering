//My Collaborative Filtering(传统的基于用户的协同过滤)
public class Application extends PrintArray implements Base {
	public static void main(String[] args) {		
		int[][] user_movie_base = new int[PREFROWCOUNT][COLUMNCOUNT];
		user_movie_base = new ReadFile().readFile(BASE_LINE, BASE); // base中有943个用户对1682个项目的评分(训练集)		
		...
		double[] rsme = new ProduceMAE().produceRSME(matrix, test);
		double Mae = 0.0, MAE = 0.0;
		double Rsme = 0.0,RSME=0.0;
		for (int k = 0; k < mae.length; k++) {
			Mae += mae[k];
		}
		for (int k = 0; k < rsme.length; k++) {
			Rsme += rsme[k];
		}
		MAE = Mae / TESTROWCOUNT;
		RSME = Rsme / TESTROWCOUNT;
